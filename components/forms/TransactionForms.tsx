...
export const GateInForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateGateInLog(editingId, logData);
        alert('Entry Updated');
        handleCancel();
      } else {
        await addGateInLog(logData);
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="xl:col-span-1">
            <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl shadow-soft border border-slate-200 sticky top-24">
                ...
                <Button type="submit" className="w-full mt-4 py-3 shadow-lg shadow-indigo-500/20">{editingId ? 'Update Record' : 'Log Movement'}</Button>
            </form>
        </div>
...
export const GateOutForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateGateOutLog(editingId, logData);
        alert('Entry Updated');
        handleCancel();
      } else {
        await addGateOutLog(logData);
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="xl:col-span-1">
            <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl shadow-soft border border-slate-200 sticky top-24">
                ...
                <Button type="submit" className="w-full mt-6 shadow-lg shadow-indigo-500/20">{editingId ? 'Update Issue' : 'Issue Stock'}</Button>
            </form>
        </div>
...
export const StockTransferForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateTransferLog(editingId, logData);
        alert('Transfer Updated');
        handleCancel();
      } else {
        await addTransferLog(logData);
