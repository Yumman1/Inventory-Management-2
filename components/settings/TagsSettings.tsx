...
export const TagsSettings = () => {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      await addOlfactiveNote(newTagName.trim());
      setNewTagName('');
    } catch (err) {
  ...
  const saveEdit = async () => {
    if (editingTag && editValue.trim()) {
      try {
        await updateOlfactiveNote(editingTag, editValue.trim());
        setEditingTag(null);
      } catch (err) {
        alert('Error: ' + (err instanceof Error ? err.message : String(err)));
      }
  ...
  const handleDelete = async (tag: string) => {
    ...
    if (window.confirm(msg)) {
      try { await deleteOlfactiveNote(tag); } catch (err) { alert('Error: ' + (err instanceof Error ? err.message : String(err))); }
    }
  };
