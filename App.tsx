...
const AppContent = () => {
  const { users, currentUser, setCurrentUser, login, signup, logout, isLoading, apiError } = useInventory();
    ...
    return (
      <div className="flex min-h-screen bg-slate-50 items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 font-bold">Loading ScentVault...</p>
        </div>
      </div>
    );
  }
  ...
  if (!currentUser && !apiError) {
    return <LoginPage onLogin={login} onSignup={signup} error={null} />;
  }

  if (apiError) {
    return (
      <div className="flex min-h-screen bg-slate-50 items-center justify-center p-8">
        <div className="bg-white rounded-2xl shadow-lg border border-rose-200 p-10 max-w-md text-center">
          <p className="text-rose-600 font-bold mb-2">Cannot connect to backend</p>
          <p className="text-slate-600 text-sm mb-4">{apiError}</p>
          <p className="text-slate-500 text-xs">Ensure the API server is running on port 4000 (npm run dev in server/)</p>
        </div>
      </div>
    );
  }
             ...
             <div className="relative">
                 ...
                 {showUserMenu && (
                     <div className="absolute right-0 mt-4 w-64 bg-white rounded-3xl shadow-2xl border border-slate-100 py-3 animate-in slide-in-from-top-4 duration-300 ring-1 ring-slate-900/5">
                         <div className="px-5 py-4 border-b border-slate-50 mb-2 flex justify-between items-center">
                             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Active Accounts</p>
                             <button
                               onClick={async () => { try { await logout(); setShowUserMenu(false); } catch (_) {} }}
                         ...
                         {users.map(u => (
                             <button 
                                key={u.id}
                                onClick={async () => { try { await setCurrentUser(u); setShowUserMenu(false); setCurrentView('dashboard'); } catch (_) {} }}
