    ...
    reader.onload = async (event) => {
      ...
      if (window.confirm("Importing a database will overwrite current local data. Continue?")) {
        try {
          ...
          alert("Error importing: " + (err instanceof Error ? err.message : String(err)));
        }
      }
    };
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
            ...
            <div className="p-5 border border-gray-100 rounded-xl bg-gray-50 flex flex-col justify-between">
                ...
                <Button onClick={async () => { try { await exportDatabase(); } catch (err) { alert('Error: ' + (err instanceof Error ? err.message : String(err))); } }} className="w-full flex items-center justify-center gap-2">
