...
export const UserForm = () => {
  ...
  const [formData, setFormData] = useState<User & { password?: string }>(initialForm);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Email and password required for login - validate on create
    if (!editingId) {
      if (!formData.email?.trim()) {
        alert('Email is required so the user can log in.');
        return;
      }
      if (!formData.password || formData.password.length < 6) {
        alert('Password is required (at least 6 characters) so the user can log in.');
        return;
      }
    }
    ...
    try {
      if (editingId) {
        const updatePayload: Partial<User> & { password?: string } = { ...userData, id: editingId };
        if (formData.password?.trim()) updatePayload.password = formData.password;
        await updateUser(editingId, updatePayload);
        setEditingId(null);
        alert('User Updated');
      } else {
        await addUser({ ...userData, id: generateId(), email: formData.email!.trim(), password: formData.password! } as User & { password: string });
        alert('User Added. Share the email and password with them so they can log in.');
      }
      setFormData({ ...initialForm, password: undefined });
      setAccessScope('all');
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
      ...
      if (window.confirm("Are you sure you want to delete this user?")) {
          try { await deleteUser(id); } catch (err) { alert('Error: ' + (err instanceof Error ? err.message : String(err))); }
      }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Password {editingId ? '(leave blank to keep current)' : '(required for login)'}
            </label>
            <input
              type="password"
